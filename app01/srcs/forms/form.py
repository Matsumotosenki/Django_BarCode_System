...
class UserModelForm(BootstrapModelForm):
    age = forms.IntegerField(required=False,
                             widget=forms.TextInput(
                                 attrs={"class": "form-control", "type": "number", "placeholder": "年龄", "min": "0",
                                        "max": "135"}))
    ...
    class Meta:  # 这里必须是Meta四个字母，不能错
        ...
        fields = ["user_id", "name", "password", "account", "age", "creat_time", "gender",
                  "department"]  # 这里必须是fields字母，不能错
        # widgets = {
...
class CodeModelForm(BootstrapModelForm):
    class Meta:
        model = CodeNumbers  # 这里必须是model字母，不能错
        fields = "__all__"  # 这里必须是fields字母，不能错
        # exclude = ["mobile"]
